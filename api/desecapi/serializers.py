...
class RRsetSerializer(ConditionalExistenceModelSerializer):
    ...
    class Meta:
        model = models.RRset
        fields = ('created', 'domain', 'subname', 'name', 'records', 'ttl', 'type', 'updated',)
        extra_kwargs = {
    ...
    def update(self, instance: models.RRset, validated_data):
        ...
        if ttl and instance.ttl != ttl:
            instance.ttl = ttl
            instance.save()  # also updates instance.updated
        else:
            # Update instance.updated without triggering post-save signal (no pdns action required)
            models.RRset.objects.filter(pk=instance.pk).update(updated=timezone.now())
